...
import { updatePet, updateUser } from "../graphql/mutations";
...
export default function UserUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = userRecord
      ? { ...initialValues, ...userRecord, Pets: linkedPets }
      : initialValues;
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : userModelProp;
      const linkedPets = record?.Pets?.items ?? [];
      setLinkedPets(linkedPets);
      setUserRecord(record);
    };
    queryData();
  }, [idProp, userModelProp]);
  React.useEffect(resetStateValues, [userRecord, linkedPets]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const linkedPetsSet = new Set();
          Pets.forEach((r) => petsSet.add(getIDValue.Pets?.(r)));
          linkedPets.forEach((r) => linkedPetsSet.add(getIDValue.Pets?.(r)));
          linkedPets.forEach((r) => {
            if (!petsSet.has(getIDValue.Pets?.(r))) {
              petsToUnLink.push(r);
            }
          });
          Pets.forEach((r) => {
            if (!linkedPetsSet.has(getIDValue.Pets?.(r))) {
              petsToLink.push(r);
            }
          });
          petsToUnLink.forEach((original) => {
            if (!canUnlinkPets) {
              throw Error(
                `Pet ${original.id} cannot be unlinked from User because userID is a required field.`
              );
            }
            promises.push(
              client.graphql({
